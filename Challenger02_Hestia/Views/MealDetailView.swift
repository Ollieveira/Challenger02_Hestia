            ...
            if showOverlay {
                ...
                VStack {
                    HStack  {
                        ...
                        CircleIconButton(systemName: "speaker.wave.2", width: 32, height: 32, font: .caption, hasNotes: false) {
                            if speechToText.getIsSpeaking() && !speechToText.getIsPaused() {
                                TelemetryManager.send("buttonPress", with: ["button": "Interrompeu a narração na MealDetailView"])
                                speechToText.stopSpeaking()
                                isReading = false
                ...
                .cornerRadius(12)
                .shadow(radius: 10)
                .offset(x: 72, y: 10) // Offset to align below the button to the right
                .frame(maxWidth: 200, alignment: .trailing) // Align to trailing edge
            }
